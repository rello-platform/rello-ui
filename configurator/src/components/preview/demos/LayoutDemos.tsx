          ...
          {items.map(item => (
            <div key={item.id}>
              <button onClick={() => toggle(item.id)} className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-[var(--neutral-50)] transition-colors">
                ...
                <span className="text-[var(--neutral-400)] transition-transform duration-200" style={{ transform: openItems.includes(item.id) ? "rotate(180deg)" : "rotate(0)" }}>▼</span>
              </button>
      ...
      <div className="m-4 bg-white rounded-lg border border-[var(--neutral-100)] divide-y divide-[var(--neutral-100)] overflow-hidden">
        {[
          ...
          <div key={item.name} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-[var(--neutral-50)] transition-colors group">
            ...
            <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded-full capitalize" style={{ backgroundColor: `var(--${item.stage}-light)`, color: `var(--${item.stage})` }}>
              <span className="size-1.5 rounded-full" style={{ backgroundColor: `var(--${item.stage})` }} />
              {item.stage}
            </span>
